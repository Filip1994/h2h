...
from quantbot.h2h_telemetry import (
    H2HOutcomeStore,
    H2HSnapshotStore,
    build_snapshot,
    h2h_snapshot_id,
...
def raw_fixture(
    fixture_id: int, when: datetime, home_goals: int = 1, away_goals: int = 0
) -> dict:
    return {
        "fixture": {
            "id": fixture_id,
...
def test_snapshot_id_is_fixture_and_cutoff_point_in_time() -> None:
    ...
    assert h2h_snapshot_id(123, cutoff) != h2h_snapshot_id(
        123, cutoff + timedelta(hours=1)
    )
...
def test_final_result_mapping_is_1x2_and_idempotent(tmp_path) -> None:
    ...
    rows = [
        json.loads(line)
        for line in (tmp_path / "data/h2h_outcomes.jsonl").read_text().splitlines()
    ]
