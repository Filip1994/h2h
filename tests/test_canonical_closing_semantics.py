...
def test_case_a_missing_closing_does_not_promote_t5(tmp_path, monkeypatch):
    ...
    entry_id = store.append_quote(_quote("2026-09-08T09:00:00+00:00", 2.0), fixture_id=1, snapshot_type="ENTRY", prediction_id="p1", bet_id="b1")
    t5_id = store.append_quote(_quote("2026-09-08T09:55:00+00:00", 1.94), fixture_id=1, snapshot_type="T5", prediction_id="p1", bet_id="b1")
    (tmp_path / "bets.json").write_text(json.dumps([{"id":"b1","event_id":1,"market":"OVER_2_5","bookmaker_id":10,"status":"WIN","kickoff":"2026-09-08T10:00:00+00:00","entry_snapshot_id":entry_id,"t5_snapshot_id":t5_id}]))
...
def test_case_b_real_closing_is_persisted_and_t5_stays_t5(tmp_path, monkeypatch):
    ...
    entry_id = store.append_quote(_quote("2026-09-08T09:00:00+00:00", 2.0), fixture_id=1, snapshot_type="ENTRY", prediction_id="p1", bet_id="b1")
    t5_id = store.append_quote(_quote("2026-09-08T09:55:00+00:00", 1.94), fixture_id=1, snapshot_type="T5", prediction_id="p1", bet_id="b1")
    (tmp_path / "bets.json").write_text(json.dumps([{"id":"b1","event_id":1,"market":"OVER_2_5","bookmaker_id":10,"status":"WIN","kickoff":"2026-09-08T10:00:00+00:00","entry_snapshot_id":entry_id,"t5_snapshot_id":t5_id,"closing_odd":1.90,"closing_opposite_odd":1.80,"closing_market_probability_devig":1/1.90/(1/1.90+1/1.80),"closing_odds_captured_at":"2026-09-08T09:59:30+00:00"}]))
...
def test_case_c_existing_intermediate_or_t5_can_never_be_promoted(tmp_path, monkeypatch):
    ...
    (tmp_path / "bets.json").write_text(json.dumps([{"id":"b1","event_id":1,"market":"OVER_2_5","bookmaker_id":10,"status":"LOSS","kickoff":"2026-09-08T10:00:00+00:00"}]))
    assert closing(_settings(tmp_path)) == 0
    snapshots = _read_snapshots(snap)
...
def test_case_d_decision_fields_are_untouched(tmp_path, monkeypatch):
    ...
    store = OddsSnapshotStore(snap)
    entry_id = store.append_quote(_quote("2026-09-08T09:00:00+00:00", 2.0), fixture_id=1, snapshot_type="ENTRY", prediction_id="p1", bet_id="b1")
    decision = {"market":"OVER_2_5","odd":2.0,"model_probability":0.58,"calibrated_probability":0.57,"ev":0.14,"edge":0.08,"h2h":{"enabled":True},"stake":1200,"risk":"PAPER","monitor_cadence":"5m","watchlist_cadence":"15m","t5_cadence":"5m"}
    bet = {"id":"b1","event_id":1,"market":"OVER_2_5","bookmaker_id":10,"status":"WIN","kickoff":"2026-09-08T10:00:00+00:00","entry_snapshot_id":entry_id,**decision,"closing_odd":1.90,"closing_opposite_odd":1.80,"closing_market_probability_devig":0.5,"closing_odds_captured_at":"2026-09-08T09:59:30+00:00"}
