...
def base_prediction(now: datetime) -> dict:
    return {
        "id": "101_UNDER_2_5_dc-value-v2.1.0",
        "event_id": 101,
        "market": "UNDER_2_5",
        "selection": "UNDER_2_5",
        "home_name": "Alpha FC",
...
def snapshot(prediction: dict, captured: datetime, odd: float, near=True, selection=None) -> dict:
    return {
        "fixture_id": prediction["event_id"],
        "prediction_id": prediction["id"],
        "market": prediction["market"],
        "selection": selection or prediction["selection"],
        ...
        "signal_class": "NEAR_MISS" if near else "OBSERVED",
        "signal_state": "NEAR_MISS" if near else "OBSERVED",
        "near_miss_reason": "EDGE_NEAR_THRESHOLD" if near else None,
...
def canonical(snapshot_row: dict, captured: datetime, snapshot_type: str) -> dict:
    return {
        "fixture_id": snapshot_row["fixture_id"],
        "prediction_id": snapshot_row["prediction_id"],
        "market": snapshot_row["market"],
        "selection": snapshot_row["selection"],
        "bookmaker_id": snapshot_row["bookmaker_id"],
        "bookmaker": snapshot_row["bookmaker"],
        "odd": snapshot_row["odd"],
        "opposite_odd": snapshot_row["opposite_odd"],
        "odds_captured_at": captured.isoformat(),
        "snapshot_type": snapshot_type,
        "observation_id": f"canon-{captured.timestamp()}",
    }
...
def test_identity_lifecycle_and_exact_bookmaker(tmp_path):
    ...
    assert row["match"] == "Alpha FC vs Beta FC"
    assert row["selection"] == "UNDER_2_5"
    assert row["bookmaker_id"] == 8
    assert row["opening_odd"] == 1.55
    assert row["closing_odd"] == 1.55
...
def test_promoted_near_miss_is_linked_without_becoming_production(tmp_path):
    ...
    assert row["promoted_to_signal_id"] == "signal-101"
    assert row["not_a_production_bet"] is True


def test_other_prediction_cannot_promote_exact_near_miss(tmp_path):
    now = datetime(2026, 9, 10, 12, tzinfo=UTC)
    prediction = base_prediction(now)
    near = snapshot(prediction, now - timedelta(minutes=10), 1.60)
    write_json(tmp_path / "predictions.json", [prediction])
...
def test_non_promoted_near_miss_expires_after_kickoff(tmp_path):
    ...
    near = snapshot(prediction, datetime(2026, 9, 10, 12, 30, tzinfo=UTC), 1.58)
    write_json(tmp_path / "predictions.json", [prediction])
    write_json(tmp_path / "bets.json", [])
    write_json(tmp_path / "intraday_alerts.json", [])
    write_json(tmp_path / "near_misses.json", [])
...
def test_settled_prediction_produces_observational_terminal_result(tmp_path):
    now = datetime(2026, 9, 10, 14, tzinfo=UTC)
    prediction = base_prediction(datetime(2026, 9, 10, 12, tzinfo=UTC))
    prediction.update({"status": "SETTLED", "outcome": 1, "result": "2:0", "settled_at": now.isoformat()})
    near = snapshot(prediction, datetime(2026, 9, 10, 12, 30, tzinfo=UTC), 1.58)
    write_json(tmp_path / "predictions.json", [prediction])
    write_json(tmp_path / "bets.json", [])
    write_json(tmp_path / "intraday_alerts.json", [])
    ...
    assert row["virtual_settled"] is True
    assert row["profit"] == 0.0


def test_transition_history_preserves_exact_identity_and_evidence(tmp_path):
    now = datetime(2026, 9, 10, 12, tzinfo=UTC)
    prediction = base_prediction(now)
    near = snapshot(prediction, now - timedelta(minutes=10), 1.60)
    ...
    write_jsonl(tmp_path / "data" / "market_timing_snapshots.jsonl", [near])
    write_jsonl(tmp_path / "data" / "odds_snapshots.jsonl", [])
    write_jsonl(tmp_path / "data" / "intraday_signal_events.jsonl", [transition])
    row = build(tmp_path, now=now)[0]
    assert len(row["transition_history"]) == 1
