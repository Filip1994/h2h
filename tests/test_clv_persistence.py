...
def test_unique_identity_and_duplicate_api_response(tmp_path):
    ...
    first = store.append_quote(quote(), fixture_id=1, snapshot_type="INTERMEDIATE", prediction_id="p1", signal_id="p1")
    duplicate = store.append_quote(quote(), fixture_id=1, snapshot_type="INTERMEDIATE", prediction_id="p1", signal_id="p1")
...
def test_identical_odds_at_different_times_are_distinct(tmp_path):
    ...
    b = store.append_quote(quote("2026-09-08T10:05:00+00:00"), fixture_id=1, snapshot_type="INTERMEDIATE")
    assert a != b
...
def test_stable_prediction_and_bet_linkage(tmp_path):
    ...
    prediction = {
        "id": "p1", "event_id": 1, "market": "OVER_2_5", "odd": 2.0,
        "opposite_odd": 1.8, "bookmaker_id": 10, "bookmaker": "Book",
        "odds_captured_at": "2026-09-08T10:00:00+00:00", "signal_id": "p1",
    }
...
def test_rejected_signal_without_bet_and_missing_bookmaker(tmp_path):
    ...
    prediction = {
        "id": "p-rejected", "event_id": 2, "market": "UNDER_2_5",
        "odd": 1.7, "opposite_odd": 2.1, "bookmaker_id": 10,
        "bookmaker": "Book", "odds_captured_at": "2026-09-08T10:00:00+00:00",
    }
...
def test_immutable_entry_and_closing_types(tmp_path):
    store = OddsSnapshotStore(tmp_path / "odds_snapshots.jsonl")
    entry = store.append_quote(quote(), fixture_id=1, snapshot_type="ENTRY", prediction_id="p1")
    assert store.append_quote(quote("2026-09-08T10:00:00+00:00", 2.2), fixture_id=1, snapshot_type="ENTRY", prediction_id="p1") is not None
    closing = store.append_quote(quote("2026-09-08T10:55:00+00:00", 1.9), fixture_id=1, snapshot_type="CLOSING", prediction_id="p1", bet_id="b1")
    records = [json.loads(x) for x in (tmp_path / "odds_snapshots.jsonl").read_text().splitlines()]
...
def test_clv_formula_correctness(tmp_path):
    store = OddsSnapshotStore(tmp_path / "odds_snapshots.jsonl")
    store.append_quote(quote("2026-09-08T10:00:00+00:00", 2.0), fixture_id=1, snapshot_type="ENTRY", prediction_id="p1")
    store.append_quote(quote("2026-09-08T10:55:00+00:00", 1.9), fixture_id=1, snapshot_type="CLOSING", prediction_id="p1", bet_id="b1")
    records = [json.loads(x) for x in (tmp_path / "odds_snapshots.jsonl").read_text().splitlines()]
