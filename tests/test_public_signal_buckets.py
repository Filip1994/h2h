...
def test_public_buckets_restore_frozen_history_and_keep_strong_ledger_separate(
    ...
    (tmp_path / "data" / "market_timing_snapshots.jsonl").write_text(
        '{"observation_id":"near-1","prediction_id":"p1","fixture_id":1,"market":"UNDER_2_5","signal_class":"NEAR_MISS","captured_at":"2026-09-09T10:00:00+00:00"}\n',
        encoding="utf-8",
    )
    ...
    assert (
        strong[0]["status"] == "WIN"
        and strong[0]["virtual_settled"] is True
    ...
    assert (
        metrics.initial_bank == 10_000.0
        and metrics.current_bank == 10_240.0
        and metrics.total_profit == 240.0
...
def test_near_miss_uses_alert_match_names_when_snapshot_has_no_team_names(
    tmp_path,
) -> None:
            ...
            [
                {
                    ...
                    "match": "Young Africans vs Geita Gold",
                    "league": "Tanzania - Ligi kuu Bara",
                }
            ]
        ),
    ...
    (tmp_path / "data").mkdir()
    (tmp_path / "data" / "market_timing_snapshots.jsonl").write_text(
        '{"observation_id":"near-1","prediction_id":"p1","fixture_id":1600773,"market":"BTTS_YES","signal_class":"NEAR_MISS","captured_at":"2026-09-09T10:00:00+00:00"}\n',
        encoding="utf-8",
    )
...
def test_explicit_virtual_settlement_is_the_only_source_of_signal_profit() -> None:
    ...
    assert (
        metrics.total_profit == 200.0
        and metrics.current_bank == 10_200.0
        and metrics.completed_count == 1
    )
