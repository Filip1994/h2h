...
class LedgerMonitor:
    """Production settlement only. Odds lifecycle is owned by the collector."""
    ...
    def run(self, now: datetime | None = None) -> dict[str, int]:
        ...
        for fixture_id in sorted(fixture_ids):
            ...
            if api_kickoff:
                for item in bets:
                    if _same_fixture(item, fixture_id) and (not item.get("kickoff") or status in {"PST", "NS", "TBD"}):
            ...
            for bet in bets:
                if not _same_fixture(bet, fixture_id) or str(bet.get("status", "")).upper() != "PENDING":
                    continue
                if status in VOID_STATUSES:
                    bet.update({"status": "VOID", "profit": 0.0, "settled_at": now.isoformat(), "result": status})
                    changed_bets = True
                    settled_bets += 1
                    continue
                if status in REVIEW_STATUSES:
                    bet.update({"status": "REVIEW", "profit": 0.0, "settled_at": now.isoformat(), "result": status})
                    changed_bets = True
                    settled_bets += 1
                    continue
                ...
                try:
                    ...
                    bet.update({"status": "REVIEW", "profit": 0.0, "settled_at": now.isoformat(), "result": f"{score[0]}:{score[1]}"})
                    changed_bets = True
                    settled_bets += 1
                    continue
                stake = float(bet.get("stake") or 0.0)
                odd = float(bet.get("odd") or 1.0)
            ...
            for prediction in predictions:
                if not _same_fixture(prediction, fixture_id) or str(prediction.get("status", "")).upper() != "PENDING":
                    continue
                if status in VOID_STATUSES:
                    prediction.update({"status": "VOID", "outcome": None, "settled_at": now.isoformat()})
                    changed_predictions = True
                    settled_predictions += 1
                    continue
                if status in REVIEW_STATUSES:
                    prediction.update({"status": "REVIEW", "outcome": None, "settled_at": now.isoformat()})
                    changed_predictions = True
                    settled_predictions += 1
                    continue
                if status not in FINISHED_STATUSES:
                ...
                if score is None:
                    ...
                    continue
                try:
                    outcome = market_outcome(Market.parse(str(prediction["market"])), *score)
                except (KeyError, ValueError):
                    continue
                prediction.update({"status": "SETTLED", "outcome": int(outcome), "result": f"{score[0]}:{score[1]}", "settled_at": now.isoformat()})
                changed_predictions = True
                settled_predictions += 1
        ...
        return {"settled_bets": settled_bets, "settled_predictions": settled_predictions, "api_requests": self.api.request_count}
...
def skip_bet(store: BetStore, identifier: str, now: datetime | None = None) -> bool:
    ...
    def mutate(bets: list[dict[str, Any]]) -> bool:
        for bet in bets:
            if str(bet.get("id")) == identifier and str(bet.get("status", "")).upper() == "PENDING":
                bet.update({"status": "SKIPPED", "profit": 0.0, "skipped_at": timestamp})
                return True
        return False
