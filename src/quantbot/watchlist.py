...
from .alerts import build_new_opportunity_email, build_signal_update_email, send_strong_signal_email
...
def _event_from_prediction(prediction: dict[str, Any], *, quote: Any, decision_probability: float, expected_value: float, probability_edge: float, stake: float, now: datetime, linked_bet: dict[str, Any] | None, signal_type: str, signal_class: str, near_miss_reason: str | None) -> dict[str, Any]:
...
def _append_transition_event(root: Path, *, prediction: dict[str, Any], quote: Any, signal_class: str, near_miss_reason: str | None, previous_class: str | None, captured_at: datetime, expected_value: float, probability_edge: float, decision_probability: float, stake: float, seconds_to_kickoff: float) -> None:
    ...
    event = {
        ...
        "transition": f"{previous_class}->{signal_class}" if previous_class else f"INITIAL->{signal_class}",
    }
...
def _apply_linked_result(event: dict[str, Any], bet: dict[str, Any] | None) -> None:
    ...
    for key in ("status", "profit", "outcome", "closing_5m_odd", "closing_5m_opposite_odd", "closing_5m_market_probability_devig", "closing_5m_odds_captured_at", "settled_at", "settlement_type"):
        if bet.get(key) is not None:
            event[f"production_{key}"] = bet[key]
    if event.get("virtual_settled") is not True:
...
def _material_improvement(current_ev: float, current_edge: float, baseline_ev: float, baseline_edge: float) -> bool:
    return current_ev - baseline_ev >= SIGNAL_UPDATE_MIN_EV_DELTA or current_edge - baseline_edge >= SIGNAL_UPDATE_MIN_EDGE_DELTA
...
def run_watchlist(settings: Settings, now: datetime | None = None) -> dict[str, int]:
    ...
    scanned = opportunities_sent = updates_sent = t5_captured = 0
    ...
    for fixture_id, fixture_predictions in by_fixture.items():
        ...
        if last_scan_raw:
            ...
            if elapsed < interval:
                continue
        try:
            quotes = extract_best_quotes(api.odds(fixture_id), bookmaker_priority=settings.bookmaker_priority, allow_any_bookmaker=settings.allow_any_bookmaker, captured_at=now_utc)
        ...
        for prediction in fixture_predictions:
            ...
            virtual_stake = kelly_stake(strong_portfolio.current_bank, decision, quote.odd, settings)
            classification = classify_signal(expected_value=ev, probability_edge=edge, stake=virtual_stake, settings=settings)
            ...
            _append_transition_event(root, prediction=prediction, quote=quote, signal_class=classification.signal_class, near_miss_reason=classification.near_miss_reason, previous_class=str(previous_class) if previous_class else None, captured_at=now_utc, expected_value=ev, probability_edge=edge, decision_probability=decision, stake=virtual_stake, seconds_to_kickoff=seconds)
            was_strong = previous_class == "STRONG_SIGNAL"
            strong = classification.signal_class == "STRONG_SIGNAL"
            snapshots.append({**build_snapshot(fixture_id=fixture_id, prediction=prediction, quote=quote, decision_probability=decision, expected_value=ev, probability_edge=edge, seconds_to_kickoff=seconds, signal_state=classification.signal_class, captured_at=now_utc), **fields, "virtual_stake": round(virtual_stake, 2)})
            ...
            if strong and linked_bet:
                baseline_ev = float(previous.get("last_alert_ev", linked_bet.get("expected_value") or 0.0))
                baseline_edge = float(previous.get("last_alert_edge", linked_bet.get("probability_edge") or 0.0))
                if _material_improvement(ev, edge, baseline_ev, baseline_edge):
                    event = _event_from_prediction(prediction, quote=quote, decision_probability=decision, expected_value=ev, probability_edge=edge, stake=virtual_stake, now=now_local, linked_bet=linked_bet, signal_type="SIGNAL_UPDATE", signal_class=classification.signal_class, near_miss_reason=classification.near_miss_reason)
                ...
                event = _event_from_prediction(prediction, quote=quote, decision_probability=decision, expected_value=ev, probability_edge=edge, stake=virtual_stake, now=now_local, linked_bet=linked_bet, signal_type="NEW_OPPORTUNITY", signal_class=classification.signal_class, near_miss_reason=classification.near_miss_reason)
                if not any(a.get("id") == event["id"] for a in alerts):
                    alerts.append(event)
            ...
            pending = [a for a in alerts if a.get("prediction_id") == key and a.get("delivery_status") == "PENDING"]
            for event in pending:
                if event not in new_opportunities and event not in signal_updates:
                    (signal_updates if event.get("signal_type") == "SIGNAL_UPDATE" else new_opportunities).append(event)
        ...
        if new_opportunities and settings.intraday_alert_enabled:
            subject, html_body = build_new_opportunity_email(new_opportunities, settings, now_local)
            if send_strong_signal_email(subject, html_body, settings):
                ...
                for event in new_opportunities:
                    event["delivery_status"] = "SENT"
                    event["delivery_sent_at"] = sent_at
        if signal_updates and settings.intraday_alert_enabled:
            subject, html_body = build_signal_update_email(signal_updates, settings, now_local)
            if send_strong_signal_email(subject, html_body, settings):
                updates_sent += len(signal_updates)
                sent_at = now_local.isoformat()
                for event in signal_updates:
                    event["delivery_status"] = "SENT"
                    event["delivery_sent_at"] = sent_at
    ...
    return {"fixtures_scanned": scanned, "opportunities_sent": opportunities_sent, "updates_sent": updates_sent, "t5_captured": t5_captured, "snapshots_appended": timing_stats["snapshots_appended"], "useful_observations": timing_stats["useful_observations"], "api_requests": api.request_count}
# fmt: on
