...
def build_packet(candidate: Any, stake: float, *, settings: Any, model: Any, decision_timestamp: datetime, calibration_hash: str | None = None) -> dict[str, Any]:
    ...
    packet = {
        ...
        "model": {
            "version": getattr(model, "MODEL_VERSION", None) or os.getenv("MODEL_VERSION", "dc-value-v2.1.0"),
            "code_sha": code_sha,
            "training_cutoff": decision_at,
            "training_sample": {
                "fitted_matches": int(getattr(model, "fitted_matches", 0)),
                "team_count": len(getattr(model, "team_ids", ()) or ()),
                "identity": _hash({"fitted_matches": int(getattr(model, "fitted_matches", 0)), "team_ids": list(getattr(model, "team_ids", ()) or ())}),
            },
        ...
        "provenance": {
            ...
            "unavailable": {"opening": "resolved_from_canonical_observation_stream", "closing": "not_available_at_decision_time"},
        },
    }
