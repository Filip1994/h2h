...
# API-Football v3 keeps league IDs stable across seasons.  This registry is
# intentionally explicit: the provider payload does not expose a reliable
# numeric tier field, so unknown IDs must fail closed instead of guessing from
# competition names.  Only competitions audited as national league tiers 1/2
# are listed here; cups, youth, reserve, amateur and lower divisions are not.
#
# Tier policy is enforced centrally in filters.py.  Add a new provider league
# only after its country, competition identity and tier have been audited.
_LEAGUES = (
    # Europe
    ...
    # African exceptions: only Egypt and Morocco are eligible.
