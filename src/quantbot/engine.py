...
class QuantEngine:
    ...
    def _prediction_record(self, fields, market, model_probability, calibrated_probability, calibration_status, h2h_rate, h2h_n, h2h_effective_n, quote, created_at, *, rejection_reason=None, selected=False):
        return {
            ...
            "event_id": fields["fixture_id"], "kickoff": fields["kickoff"].isoformat(), "created_at": created_at.isoformat(),
            "decision_timestamp": created_at.astimezone(UTC).isoformat(), "data_cutoff": created_at.astimezone(UTC).isoformat(),
            "league_id": fields["league_id"], "league": f"{fields['country']} - {fields['league_name']}",
            "home_id": fields["home_id"], "away_id": fields["away_id"], "match": f"{fields['home_name']} vs {fields['away_name']}",
            "market": market.value, "model_probability": round(model_probability, 6), "calibrated_probability": round(calibrated_probability, 6),
            "calibration_status": calibration_status, "h2h_enabled": self.settings.h2h_telemetry_enabled,
            "h2h_rate": round(h2h_rate, 6), "h2h_n": h2h_n, "h2h_effective_n": round(h2h_effective_n, 3),
            "h2h_eligible": h2h_rate >= self.settings.min_h2h_rate if h2h_n else None,
            "odd": round(quote.odd, 4) if quote else None, "opposite_odd": round(quote.opposite_odd, 4) if quote else None,
            "bookmaker_id": quote.bookmaker_id if quote else None, "bookmaker": quote.bookmaker_name if quote else None,
