...
def _build_signal_email(
    ...
    *,
    ...
    for bet in strong:
        ...
        logo_html = (
            f'<img src="{_esc(logo)}" width="28" height="28" alt="" '
        ...
        cards.append(
            f'''<table width="100%" cellspacing="0" cellpadding="0" style="background:#0d1a2b;border:1px solid #244c67;border-radius:16px;margin:0 0 14px;"><tr><td style="padding:18px;"><div style="font-size:10px;letter-spacing:2px;color:#ffbd5c;font-weight:900;">{_esc(eyebrow)}</div><div style="font-size:18px;font-weight:900;color:#fff;margin-top:6px;">⚽ {_esc(bet.get("match"))}</div><div style="font-size:12px;color:#7f9ab8;margin:6px 0 14px;">{_esc(bet.get("league"))} · kickoff {_esc(kickoff)}</div><div style="font-size:13px;color:#c4d7e8;">{_esc(bet.get("market_display", bet.get("market")))} · <b style="color:#45d9ff;">@ {float(bet.get("odd") or 0):.2f}</b></div><div style="margin-top:10px;font-size:12px;color:#a9bfd5;">MODEL {100 * float(bet.get("model_probability") or 0):.1f}% · DECISION {100 * float(bet.get("decision_probability") or 0):.1f}% · EV <b style="color:#45f0a5;">{100 * float(bet.get("expected_value") or 0):+.1f}%</b> · EDGE <b style="color:#45f0a5;">{100 * float(bet.get("probability_edge") or 0):+.1f}pp</b></div><div style="margin-top:14px;padding-top:12px;border-top:1px solid #203b5c;">{logo_html}<b style="color:#eef7ff;">{_esc(bet.get("bookmaker") or "Bookmaker")}</b> · <b style="color:#45f0a5;">{_money(float(bet.get("stake") or 0))} RSD</b></div><div style="margin-top:12px;"><a href="{_esc(_skip_url(settings.github_repository, str(bet.get("id"))))}" style="color:#ff8a9a;text-decoration:none;font-size:10px;font-weight:800;">↳ PRESKOČI TIP</a></div></td></tr></table>'''
        )
    body = f'''<!doctype html><html><body style="margin:0;background:#050b14;font-family:Arial,Helvetica,sans-serif;color:#eef7ff;"><table width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:24px 10px;"><table width="680" cellspacing="0" cellpadding="0" style="max-width:680px;width:100%;background:#081321;border:1px solid #193451;border-radius:20px;overflow:hidden;"><tr><td style="padding:26px 24px;background:#0b1929;border-bottom:1px solid #193451;"><div style="font-size:10px;letter-spacing:3px;color:#5f87ab;font-weight:800;">QUANTBET // INTRADAY INTELLIGENCE</div><div style="font-size:25px;font-weight:900;margin-top:7px;">🚨 {_esc(kind)}</div><div style="font-size:12px;color:#7f9ab8;margin-top:7px;">{generated_at:%d.%m.%Y · %H:%M} · {_esc(source_label)} · {len(strong)} QUALIFIED</div></td></tr><tr><td style="padding:18px 20px;">{"".join(cards) or '<div style="padding:18px;color:#7f9ab8;">No signal crossed the alert threshold.</div>'}</td></tr><tr><td style="padding:14px 20px 22px;color:#718aa4;font-size:10px;line-height:1.7;">Alert threshold: EV ≥ 10%, probability edge ≥ 5pp, stake ≥ 300 RSD. Daily picks are monitored separately; near-miss candidates can become new opportunities.</td></tr></table></td></tr></table></body></html>'''
    subject = f"🚨 QuantBet {kind}: {len(strong)} · {generated_at:%H:%M}"
    return subject, body


def build_new_opportunity_email(
    bets: list[dict[str, Any]], settings: Settings, generated_at: datetime
) -> tuple[str, str]:
    strong = [bet for bet in bets if is_strong_signal(bet, settings)]
    return _build_signal_email(
        strong,
        settings,
        generated_at,
        ...
        source_label="WATCHLIST · NEAR-MISS",
    )


def build_signal_update_email(
    bets: list[dict[str, Any]], settings: Settings, generated_at: datetime
) -> tuple[str, str]:
    strong = [bet for bet in bets if is_strong_signal(bet, settings)]
    return _build_signal_email(
        ...
        generated_at,
        kind="SIGNAL UPDATE",
        eyebrow="📈 SIGNAL UPDATE",
        source_label="WATCHLIST · EXISTING PICK",
    )
...
def send_strong_signal_email(subject: str, html_body: str, settings: Settings) -> bool:
    if not settings.gmail_user or not settings.gmail_app_pass or not settings.email_to:
        print(
            "⚠️ Gmail secrets nisu podešeni; strong-signal alert je sačuvan bez slanja emaila."
        )
        return False
