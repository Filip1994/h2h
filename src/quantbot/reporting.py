...
<tr><td style="padding:16px 18px 8px;"><div style="font-size:16px;font-weight:800;color:#eef7ff;">⚽ {_esc(bet["match"])}</div><div style="font-size:11px;color:#7f9ab8;margin-top:5px;">{_esc(bet["league"])} · KICKOFF {kickoff:%d.%m.%Y %H:%M}</div></td></tr>
<tr><td style="padding:5px 18px 12px;"><span style="font-size:10px;letter-spacing:1px;color:#6f8eac;">MARKET</span><br><b style="font-size:15px;color:#ffffff;">{_esc(bet["market_display"])}</b> <span style="color:#45d9ff;font-weight:800;">@ {float(bet["odd"]):.2f}</span></td></tr>
<tr><td style="padding:0 18px 12px;color:#a9bfd5;font-size:12px;">MODEL <b style="color:#eef7ff;">{100 * float(bet["model_probability"]):.1f}%</b> · DECISION <b style="color:#eef7ff;">{100 * float(bet["decision_probability"]):.1f}%</b> · EV <b style="color:{roi_color};">{100 * float(bet["expected_value"]):+.1f}%</b></td></tr>
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#050b14;"><tr><td align="center" style="padding:24px 10px;">
<table role="presentation" width="680" cellspacing="0" cellpadding="0" style="max-width:680px;width:100%;background:#081321;border:1px solid #193451;border-radius:20px;overflow:hidden;">
...
<tr><td style="padding:18px 20px;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr>
<td width="25%" style="padding:12px 8px;background:#0d1a2b;border:1px solid #203b5c;border-radius:10px;text-align:center;"><div style="font-size:9px;color:#6f8eac;letter-spacing:1px;">BANK</div><b style="font-size:15px;">{_money(analytics.current_bank)} RSD</b></td>
<td width="25%" style="padding:12px 8px;background:#0d1a2b;border:1px solid #203b5c;border-radius:10px;text-align:center;"><div style="font-size:9px;color:#6f8eac;letter-spacing:1px;">ROI</div><b style="font-size:15px;color:{roi_color};">{100 * analytics.roi:+.2f}%</b></td>
<td width="25%" style="padding:12px 8px;background:#0d1a2b;border:1px solid #203b5c;border-radius:10px;text-align:center;"><div style="font-size:9px;color:#6f8eac;letter-spacing:1px;">WIN RATE</div><b style="font-size:15px;">{100 * analytics.win_rate:.1f}%</b></td>
<td width="25%" style="padding:12px 8px;background:#0d1a2b;border:1px solid #203b5c;border-radius:10px;text-align:center;"><div style="font-size:9px;color:#6f8eac;letter-spacing:1px;">DRAWDOWN</div><b style="font-size:15px;">{100 * analytics.current_drawdown:.1f}%</b></td>
</tr></table></td></tr>
<tr><td style="padding:0 20px 6px;"><div style="font-size:11px;letter-spacing:2px;color:#5f87ab;font-weight:800;">TODAY'S SIGNALS</div></td></tr>
<tr><td style="padding:8px 20px 12px;">{picks_html}</td></tr>
