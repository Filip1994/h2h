...
def build_snapshot(
    *,
    ...
    home_name = prediction.get("home_name") or prediction.get("home_team") or prediction.get("home")
    away_name = prediction.get("away_name") or prediction.get("away_team") or prediction.get("away")
    match = prediction.get("match") or (f"{home_name} vs {away_name}" if home_name and away_name else None)
    snapshot = {
...
def append_snapshots(root: Path, snapshots: list[dict[str, Any]], *, api_requests: int) -> dict[str, int]:
    if not snapshots:
        ...
        with canonical_path.open("r", encoding="utf-8", errors="replace") as handle:
            for line in handle:
                try:
                    ...
                    continue
                if isinstance(row, dict) and row.get("snapshot_type") == "ENTRY" and row.get("prediction_id"):
    ...
    with path.open("a", encoding="utf-8") as handle:
        for snapshot in snapshots:
            ...
            appended += 1
            if snapshot.get("signal_state") in {"NEAR_MISS", "STRONG", "STRONG_SIGNAL", "SIGNAL_UPDATE"}:
                useful += 1
            try:
                ...
                base = {
                    ...
                    "source_request_hash": source_request_hash("odds", {"fixture": int(snapshot["fixture_id"])}),
                    "captured_by": "watchlist",
                    "home_name": snapshot.get("home_name"),
                    "away_name": snapshot.get("away_name"),
                ...
                if snapshot.get("signal_state") in {"STRONG", "STRONG_SIGNAL"} and prediction_id and prediction_id not in entry_prediction_ids:
    ...
    metrics["api_requests_per_useful_observation"] = round(api_requests / max(1, useful), 4)
