...
def main() -> int:
    ...
    if not isinstance(run_result, dict):
        payload = {"schema_version": 1, "status": "ERROR", "checked_at": datetime.now(UTC).isoformat(), "errors": ["WATCHLIST_RESULT_NOT_VALID_JSON_OBJECT"], "observation_only": True}
        args.output.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        return 1
    if not isinstance(predictions, list):
        predictions = []
    payload = evaluate(run_result, [item for item in predictions if isinstance(item, dict)], now=datetime.now(UTC), lookahead_hours=args.lookahead_hours)
    args.output.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
