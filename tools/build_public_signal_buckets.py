...
def _legacy_settlement_map(root: Path) -> dict[str, dict[str, Any]]:
    ...
    return {
        _canonical_key(row): row for row in rows
        if _canonical_key(row) and str(row.get("status") or "").upper() in TERMINAL_STATUSES
    }
...
def _restore_legacy_virtual_settlement(row: dict[str, Any], settlements: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    row.update({"status": status, "virtual_status": status, "profit": profit,
                "virtual_profit": profit, "virtual_settled": True})
...
def merge(items: list[dict[str, Any]], settlements: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        if was_settled and item.get("virtual_settled") is not True:
            protected = {k: current.get(k) for k in (
...
def _public_strong_event(row: dict[str, Any], source: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    out.setdefault("virtual_settled", str(out.get("status") or "PENDING").upper() in TERMINAL_STATUSES)
    if out["virtual_settled"]:
        out["virtual_status"] = str(out.get("status") or "PENDING").upper()
        out["virtual_profit"] = float(out.get("virtual_profit") or out.get("profit") or 0.0)
...
def build(root: Path = ROOT) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    (root / "strong_signals_portfolio.json").write_text(
        json.dumps({
            "portfolio": STRONG_SIGNAL_PORTFOLIO, "production": False,
            "not_a_production_bet": True, "initial_bank": metrics.initial_bank,
            "current_bank": metrics.current_bank, "total_profit": metrics.total_profit,
            "total_stake": metrics.total_stake, "roi": metrics.roi,
            "win_rate": metrics.win_rate, "completed_count": metrics.completed_count,
            "open_stake": metrics.open_stake, "current_drawdown": metrics.current_drawdown,
