    ...
    for index, row in enumerate(rows):
        ...
        for field in (
            ...
            assert str(row.get(field) or "").strip(), (
                f"near_misses.json[{index}]: missing canonical {field}"
            )
        status = str(row.get("status") or "").upper()
        assert status in STATUS_VALUES, (
            f"near_misses.json[{index}]: invalid lifecycle status {status!r}"
        )
        if status in {"PENDING", "PROMOTED", "SETTLEMENT_PENDING"}:
            assert row.get("kickoff"), (
                f"near_misses.json[{index}]: active row missing kickoff"
            )
...
def main() -> int:
    ...
    print(
        f"PASS public dashboard contract: bets={len(bets)} strong={len(strong)} near={len(near)} strong_asset={referenced}"
    )
    return 0
