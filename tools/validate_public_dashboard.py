...
def main() -> int:
    ...
    for name, page_key in PAGES.items():
        ...
        assert './assets/qb-dashboard.js' in text and './assets/qb-dashboard.css' in text
    ...
    for page in PAGES.values():
        assert f"page==='{'overview' if page == 'overview' else page}'" in js or page in js, f"dashboard renderer missing {page}"
    ...
    print(f"PASS public dashboard contract: bets={len(bets)} strong={len(strong)} near={len(near)}")
    return 0
