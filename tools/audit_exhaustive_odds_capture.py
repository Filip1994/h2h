...
def main() -> int:
    ...
    for row in history if isinstance(history, list) else []:
        ...
        errors = int(row.get("api_error_events") or 0) + int(
            row.get("network_error_events") or 0
        )
    ...
    for date in sorted(by_day)[-7:]:
        ...
        print(
            f"- {date}: requests={item['requests']}, odds_requests={item['odds']}, fixture_requests={item['fixtures']}, errors={item['errors']}, working_budget={max(0, budget - 500)}"
        )
    print()

    print("## Fixture-universe coverage")
    print(
        "The current committed generation ledger does not persist a complete eligible Football fixture universe or per-fixture odds-query outcome. Therefore queried/eligible coverage cannot be reconstructed honestly from main without querying the provider. This is a confirmed observability gap, not a zero-coverage claim."
    )
    print()
    ...
    fixtures = {
        row.get("fixture_id") for row in snapshots if row.get("fixture_id") is not None
    }
    keys = {
    ...
    for row in (
        [*predictions, *bets]
        if isinstance(predictions, list) and isinstance(bets, list)
        else []
    ):
    ...
    print(
        f"Ledger records without an entry odds timestamp: {missing.get('NO_ENTRY_OBSERVATION', 0)}"
    )
    print()
    ...
    for name, rows in (("Production bets", bets), ("Predictions", predictions)):
        ...
        opening = sum(
            1
            for row in rows
            if isinstance(row, dict) and row.get("opening_odd") is not None
        )
        closing = sum(
            1
            for row in rows
            ...
            and (
                row.get("closing_odd") is not None
                or row.get("closing_5m_odd") is not None
            )
        )
        print(f"- {name}: total={total}, opening={opening}, closing={closing}")
    print(
        "Strong Signal lifecycle is not separately auditable from the committed state because the public signal ledger does not yet persist a complete immutable observation coverage summary."
    )
    print()
    ...
    print(
        "Workflow schedule is not equivalent to successful odds coverage; actual scan timestamps must be persisted by the new collector."
    )
    print()

    print("## Provider availability by time-to-kickoff")
    print(
        "Not measurable from the current committed observation ledger because fixture kickoff and every odds response are not joined into a fixture-level collection state. The new collector must persist explicit NO_ODDS_RESPONSE outcomes so provider availability is measured rather than inferred."
    )
    print()

    print("## Phase 0 conclusion")
    print(
        "PASS — the audit identifies a material API-capacity utilization gap and a missing fixture-level coverage ledger. No model, EV, edge, Kelly, risk or eligibility thresholds are changed by this audit."
    )
    return 0
